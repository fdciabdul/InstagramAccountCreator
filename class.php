...
class instaCreator {
	...
	protected $csrf_token = '4j78Wn7q2dd72wyXLred0Llz7tudss1g';
	protected $mid_token = 'XKnIoAABAAGfcQxrne0VokoQF05f';
	...
	public function userCreate($count = 1, $sleep = 10){
		do {
			for($i = 1; $i < $count; $i++){
				...
				$randomUser_Password = "abdulmuttaqin";
				...
				if($userCreate->account_created == "true"){
					...
					fclose($file);
					echo "[!] ".$i.". Berhasil Membuat Akun: ".$randomUser_User_Name.":".$randomUser_Password."\n";
				}else{
					echo "[!] ".$i.". Gagal Membuat Akun: ".$randomUser_User_Name.":".$randomUser_Password."\n";
				}
				sleep($sleep);
			}
